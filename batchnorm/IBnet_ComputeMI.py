...
MAX_EPOCHS = 4000      # Max number of epoch for which to compute mutual information measure
NUM_LABELS = 2
# MAX_EPOCHS = 1000
COLORBAR_MAX_EPOCHS = 4000
...
binsize = 0.30                           # size of bins for binning method
...
for activation in measures.keys():
    cur_dir = 'rawdata/' + DIR_TEMPLATE % activation + '_' + 'bn'
    ...
    for epochfile in sorted(os.listdir(cur_dir)):
        ...
        for lndx in range(num_layers):
            ...
            # sz = (np.max(activity)-np.min(activity))/10.0
            sz = np.std(activity)*1.0
